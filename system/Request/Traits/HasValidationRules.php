...
trait HasValidationRules
{
    ...
    {
        if ($this->checkFieldExist($name) && $this->checkFirstError($name) && !filter_var($this->request[$name], FILTER_VALIDATE_EMAIL)) {
    ...
    {
        if ($this->checkFieldExist($name) && !preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/", $this->request[$name])
            && $this->checkFirstError($name)) {
    ...
    {
        if ($this->checkFieldExist($name) && !is_numeric($this->request[$name]) && $this->checkFirstError($name)) {
